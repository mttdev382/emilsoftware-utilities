...
export class UserController {
  ...
  @ApiResponse({
    ...
    description:
      "Errore interno del server durante la registrazione o l’invio dell’email.",
  })
  @Post("register")
  async register(
    @Req() request: Request,
  ...
  @ApiBody({
    type: UserDto,
    description:
      "Dati aggiornati dell'utente (escluso il codice utente, che è nel path)",
  })
  @ApiResponse({
  ...
  async setGdpr(
    ...
    try {
      ...
      return RestUtilities.sendOKMessage(
        res,
        `L'utente ${codiceUtente} ha accettato il GDPR.`
      );
    } catch (error) {
      return RestUtilities.sendErrorMessage(res, error, UserController.name);
    }
